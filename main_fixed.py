...
# ─────────────────────────────────────────────
# DOWNLOAD MODEL (needed for new MediaPipe)
# ─────────────────────────────────────────────
...
def main():
    ...
    # ── MediaPipe FaceLandmarker (new API — works with mediapipe 0.10.30+) ──
    ...
    mp_face_mesh = vision.FaceLandmarker.create_from_options(fl_options)

    # ── Second FaceLandmarker — dedicated to pupil detection (more accurate) ──
    pupil_base    = python.BaseOptions(model_asset_path=MODEL_PATH)
    pupil_options = vision.FaceLandmarkerOptions(
        base_options=pupil_base,
        output_face_blendshapes=False,
        output_facial_transformation_matrixes=False,
        num_faces=1,
    )
    pupil_detector = vision.FaceLandmarker.create_from_options(pupil_options)
    ...
    try:
        while True:
            ...
            results        = mp_face_mesh.detect(mp_image)
            pupil_results  = pupil_detector.detect(mp_image)
            ...
            if results.face_landmarks:
                ...
                mesh_3d = np.array([[n.x, n.y, n.z] for n in lm])
                ...
                # iris circles hidden — yellow pupil circles shown instead
                ...
                # ── Pupil size (uses dedicated FaceLandmarker — more accurate) ───
                if pupil_results.face_landmarks:
                    p_lm    = pupil_results.face_landmarks[0]
                    mesh_pupil = np.array(
                        [[int(p.x * img_w), int(p.y * img_h)] for p in p_lm]
                    )

                    l_res = get_pupil_radius(frame, mesh_pupil, LEFT_EYE_CROP)
                    r_res = get_pupil_radius(frame, mesh_pupil, RIGHT_EYE_CROP)

                    if (
                        l_res is not None and r_res is not None and
                        l_res[2] is not None and r_res[2] is not None
                    ):
                        l_p_cx, l_p_cy, l_radius = l_res
                        r_p_cx, r_p_cy, r_radius = r_res
                        ...
                        # classify each eye separately (more accurate than averaging)
                        left_trend  = classify_pupil_change(left_radius_history)
                        right_trend = classify_pupil_change(right_radius_history)
                        pupil_trend = left_trend if left_trend != "Stable" else right_trend

                        # draw yellow circles around actual pupils
                        cv.circle(frame, (l_p_cx, l_p_cy), int(l_radius), (0, 255, 255), 2, cv.LINE_AA)
                        cv.circle(frame, (r_p_cx, r_p_cy), int(r_radius), (0, 255, 255), 2, cv.LINE_AA)
        ...
        # FaceLandmarker closes automatically
